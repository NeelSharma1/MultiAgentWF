...
Use **Accounts** in the app header to connect Codex with ChatGPT device authorization or securely save/remove OpenAI, Gemini, Anthropic, and OpenAI-compatible endpoint credentials. Saved keys are written only to the ignored `.env.local` with owner-only file permissions and are never returned to the frontend.
...
- `toolsets.py`: project-local toolset manifests, summary documents, assignments, `TOOLCALL` parsing, safe process launch, and chat-result formatting.
